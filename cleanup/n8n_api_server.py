#!/usr/bin/env python3
"""
API Server final para n8n - Genera gráficos de velas de OKX
"""
...
def create_dataframe(data):
    ...
    # Convertir precios de manera segura - manejar cada columna individualmente
    numeric_columns = ['open', 'high', 'low', 'close']
    for col in numeric_columns:
        # Limpiar y convertir cada valor individualmente
        cleaned_values = []
        for value in df[col]:
            try:
                # Si el valor es una cadena que contiene múltiples números, tomar el primero
                if isinstance(value, str) and len(value) > 20:
                    # Buscar el primer número en la cadena
                    import re
                    numbers = re.findall(r'\d+\.?\d*', value)
                    if numbers:
                        cleaned_values.append(float(numbers[0]))
                    else:
...
def health_check():
    ...
    return jsonify({
        "status": "healthy",
        "timestamp": datetime.now().isoformat(),
        "service": "OKX Candlestick Chart API for n8n",
        "version": "1.0.0"
    })
...
if __name__ == '__main__':
    print("🚀 Iniciando API Server para n8n - Gráficos de Velas OKX")
    print("=" * 60)
    ...
    print()
    print("🔗 El servidor estará disponible en: http://localhost:5002")
    print("📝 Para n8n, usa: http://localhost:5002/generate-chart")
    print()
